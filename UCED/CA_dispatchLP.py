...
#Emission taxs
model.CO2Tax = Param(model.Generators)
model.NOXTax = Param(model.Generators)
model.SO2Tax = Param(model.Generators)
model.PMTax = Param(model.Generators)
...
def SysCost(model):
    ...
    coal1 = sum(model.mwh_1[j,i]*(model.seg1[j]*2 + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Coal)
    coal2 = sum(model.mwh_2[j,i]*(model.seg2[j]*2 + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Coal)
    coal3 = sum(model.mwh_3[j,i]*(model.seg3[j]*2 + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Coal)
    gas1_1 = sum(model.mwh_1[j,i]*(model.seg1[j]*model.GasPrice['PGE_valley'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone1Gas)
    gas2_1 = sum(model.mwh_2[j,i]*(model.seg2[j]*model.GasPrice['PGE_valley'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone1Gas)
    gas3_1 = sum(model.mwh_3[j,i]*(model.seg3[j]*model.GasPrice['PGE_valley'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone1Gas)
    gas1_2 = sum(model.mwh_1[j,i]*(model.seg1[j]*model.GasPrice['PGE_bay'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone2Gas)
    gas2_2 = sum(model.mwh_2[j,i]*(model.seg2[j]*model.GasPrice['PGE_bay'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone2Gas)
    gas3_2 = sum(model.mwh_3[j,i]*(model.seg3[j]*model.GasPrice['PGE_bay'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone2Gas)
    gas1_3 = sum(model.mwh_1[j,i]*(model.seg1[j]*model.GasPrice['SCE'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone3Gas)
    gas2_3 = sum(model.mwh_2[j,i]*(model.seg2[j]*model.GasPrice['SCE'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone3Gas)
    gas3_3 = sum(model.mwh_3[j,i]*(model.seg3[j]*model.GasPrice['SCE'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone3Gas)
    gas1_4 = sum(model.mwh_1[j,i]*(model.seg1[j]*model.GasPrice['SDGE'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone4Gas)
    gas2_4 = sum(model.mwh_2[j,i]*(model.seg2[j]*model.GasPrice['SDGE'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone4Gas)
    gas3_4 = sum(model.mwh_3[j,i]*(model.seg3[j]*model.GasPrice['SDGE'] + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Zone4Gas)
    ...
    oil1 = sum(model.mwh_1[j,i]*(model.seg1[j]*20 + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Oil)
    oil2 = sum(model.mwh_2[j,i]*(model.seg2[j]*20 + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Oil)
    oil3 = sum(model.mwh_3[j,i]*(model.seg3[j]*20 + model.var_om[j] + model.CO2Tax[j] + model.NOXTax[j] + model.SO2Tax[j] + model.PMTax[j]) for i in model.hh_periods for j in model.Oil)
